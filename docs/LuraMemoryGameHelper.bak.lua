...
local function refreshSettingsControls()
    ...
    if settingsFrame.modeDropdown then
        UIDropDownMenu_SetText(settingsFrame.modeDropdown, getDifficultyModeLabel(manualDifficultyOverride))
    end
end
...
local function buildSettingsWindow()
    ...
    f.autoFillCB = createCheckbox("Enable Autofill", 18, -42, function(self)
        AUTO_FILL_SLOT5 = self:GetChecked() and true or false
        saveSettingsState()
    end)

    f.dupCB = createCheckbox("Prevent Duplicates", 18, -72, function(self)
        PREVENT_DUPLICATES = self:GetChecked() and true or false
        saveSettingsState()
    end)

    f.lockCB = createCheckbox("Lock After Send", 18, -102, function(self)
        LOCK_AFTER_SEND = self:GetChecked() and true or false
        saveSettingsState()
    end)

    f.rwCB = createCheckbox("Also Send Raid Warning", 18, -132, function(self)
        if not playerCanBroadcast() then
            self:SetChecked(false)
            return
        end
        alsoSendRW = self:GetChecked() and true or false
        saveSettingsState()
